...
let ret = npc.askYesNo("你做好决定了吗? 选择了将不可以反悔了, 所以你要好好想清楚. 你现在就想成为一个#r爆破手#k吗?");
if (ret == 1) {
        if (player.canGainItem(1582000, 1)) {
                npc.completeQuest();
                if (player.getJob() == 3000) {
                        ...
                        npc.sayNext("我刚刚重塑了你的身体，使你成为一个完美的爆破手。 如果你想变得更加强大，使用属性窗口（S键）来提升你的属性。若不清楚的话, 不妨使用#b自动分配#k也好。");
                        npc.sayNext("我还扩充了你背包的装备栏和其他栏，好好利用, 你可以收集更多的武器和防具, 让自己变得更加强大。！");
                        npc.sayNext("对了，有一点需要注意。虽然预备兵的时候没关系，但是成为爆破手的瞬间开始，必须小心不要死掉……万一死了的话，之前积累的经验值可能会受到损失……");
