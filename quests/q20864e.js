...
function endScript() {
        let ret = npc.askYesNo("你做好选择了吗?一旦选择后将无法更改, 所以做选择前请慎重.你想要成为一个#b夜行者#k吗?");
        if (ret == 1) {
                if (player.getJob() == 1000) {
                        player.gainItem(1472061, 1);
                        player.gainItem(2070015, 800);
                        player.gainItem(2070015, 800);
                ...
                npc.completeQuest();
                npc.sayNext("我刚刚重塑了你的身体，使你成为一个完美的夜行者。 如果你想变得更加强大，使用属性窗口（S键）来提升你的属性。若不清楚的话, 不妨使用#b自动分配#k也好。");
                npc.sayNext("我还扩充了你背包的装备栏和其他栏，好好利用, 你可以收集更多的武器和防具, 让自己变得更加强大。！");
                npc.sayNext("对了，有一点需要注意。虽然初心者的时候没关系，但是成为夜行者的瞬间开始，必须小心不要死掉……万一死了的话，之前积累的经验值可能会受到损失……");
