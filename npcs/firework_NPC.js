...
var totalCount=getEventCount("牛年捐赠");
var sql="select sum(value) total from accounts_event where `event`='牛年捐赠'";
var worldCount=player.customSqlResult(sql).get(0).get("total");
var txt="\t\t\t#r#e#v"+need+"#牛年装扮大作战#v"+need+"#\r\n\r\n";
txt+=" 你好，新年快到了，不得放点爆竹吗？\r\n\r\n";
txt+="你可以帮我去收集#v"+need+"#来给我吗？\r\n\r\n";
		...
		if(item==null){
			...
			if(ch==0){
				...
				if(worldCount>500000){
					if(getEventCount("牛年收集50万奖励")==0){
						//5W的奖励
						
						addHyPay(100);
						setEventCount("牛年收集50万奖励",0,1)
						npc.say("领取成功");
					}else{
						npc.say("请不要重复领取");
					}
