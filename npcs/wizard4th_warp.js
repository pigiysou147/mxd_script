...
} else if (player.getJob() < 200 || player.getJob() > 232) {
        npc.sayNext("你为什么想见我？我没有你想知道的事。");
} else if (player.isQuestStarted(1453)) {
        let sel = npc.askMenu("我可以将你传送到天鹰或火焰龙所在地, 那么你想去 \r\n #b#L0#火焰龙森林#l\r\n #b#L1#天鹰森林#l");
        if (sel == 0) {
                if (player.hasItem(4031511, 1)) {
                        npc.sayNext("你已经有#b#t4031511##k了");
                } else {
                        npc.makeEvent("change_job", false, [player, 924000200]);
                }
        } else if (sel == 1) {
                if (player.hasItem(4031512, 1)) {
                        npc.sayNext("你已经有#b#t4031512##k了");
                } else {
                        npc.makeEvent("change_job", false, [player, 924000201]);
                }
        }
} else {
        npc.sayNext("你还不够强大走魔法师顶尖的道路。等你变得更强壮再来找我吧。");
}