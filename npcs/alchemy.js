...
let selStr = "你好。你对炼金术感兴趣吗？\r\n\r\n";
if (player.getMakingSkillLevel(92040000) > 0) {
        selStr += "#L2##b提升#e炼金术#n等级。#l\r\n#L3#炼金术技术初始化。#k#l";
} else {
...
switch (sel) {
        case 0:
                npc.sayNext("炼金术是用草药的精油制作各种药水的技术。除了恢复HP和MP的药水之外，还可以制作各种让你变强的药水。还能制作你从来没有体验过的神奇药水。");
                break;
        case 1:
                if (player.getLevel() < 30) {
                        npc.say("#b至少必须达到30级2转以上，龙神必须3转以上，暗影双刀必须2转+以上#k，才能学习专业技术。你能达到条件之后再来找我吗？");
                } else if (player.getMakingSkillLevel(92040000) > 0) {
                        npc.say("你已经学些过#e炼金术#n，难道还想学？");
                } else if (!player.isQuestCompleted(3199)) {
                        npc.say("你对炼金术感兴趣吗？想学的话，必须先去见见#b希梅尔#k～在村子入口处就能见到希梅尔了。");
                } else if (getMakingSkillSize() > 0) {
                        npc.say("嗯，你好像已经学习了其他的制作技术。真想学习的话，就必须先放弃当前的制作技术。");
                } else if (player.getMakingSkillLevel(92000000) <= 0) {
                        npc.say("炼金术必须先学习采药。往右边走，可以看到在大锅旁用心提炼草药的采药大师#b斯塔切#k，你可以向她学习采药。");
                } else {
                        let ret = npc.askYesNo("你真的要学习#b炼金术#k吗？\r\n学费是#b5000金币#k。\r\n#b");
                        if (ret == 1) {
                                if (player.hasMesos(5000)) {
                                        player.loseMesos(5000);
                                        player.setSkillLevel(92040000, 0x1000000, 12, false);
                                        player.soundEffect("profession/levelup");
                                        player.showAvatarOriented("Effect/OnUserEff.img/professions/alchemy_levelup");
                                        npc.say("好！非常好！太棒了！！好了～我已经把炼金术知识全部传授给你了。熟练度满了之后，可以提升炼金术等级。别忘了到时再来找我。");
                                } else {
                                        npc.say("你的钱不够啊。想要学习的话，需要#b5000金币#k才行。这可是规矩。");
                                }
                        } else {
                                npc.say("什么！！！炼金术是非常有用的技术。这都还要考虑？你也太慎重了吧？");
                        }
                }
                break;
        case 2:
                if (player.getMakingSkillLevel(92040000) >= 12) {
                        ...
                        if (ret == 1) {
                                ...
                                player.soundEffect("profession/levelup");
                                player.showAvatarOriented("Effect/OnUserEff.img/professions/alchemy_levelup");
                                npc.sayNext("好了，我已经帮你提升了炼金术等级。熟练度积满后可再提升至下一等级，就可以学习新的内容。等你积满熟练度后，再来找我吧。");
                        } else {
                ...
                break;
        case 3:
                let ret = npc.askYesNo("你想放弃炼金术？是厌倦了吗？之前积累的等级和熟练度……付出的努力和金钱……都将会变成泡影……你真的要初始化吗？");
                if (ret == 1) {
