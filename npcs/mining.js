...
switch (sel) {
        case 0:
                npc.sayNext("采矿是用十字镐之类的工具，采集地图上的矿石的技能。采集到的矿石，可以用#p9031007#出售的铁砧进行冶炼，获得装备、饰品、炼金术所需的材料。");
                break;
        case 1:
                if (player.getLevel() < 30) {
                        npc.say("小毛孩！你还不够强，还不能学习专业技术。#b至少必须达到30级2转以上，龙神必须3转以上，暗影双刀必须2转+以上#k，才能学习专业技术。等达到条件之后再来找我吧。");
                } else if (player.getMakingSkillLevel(92010000) > 0) {
                        npc.say("你已经学些过#e采矿#n，难道还想学？");
                } else if (!player.isQuestCompleted(3199)) {
                        npc.say("你对采矿感兴趣吗？想学的话，必须先去见见#b希梅尔#k～在村子入口处就能见到希梅尔了。");
                } else if (getMakingSkillSize() > 2) {
                        npc.say("嗯，你好像已经学习了3种专业技术。真想学习的话，就必须先放弃一种技术。");
                } else {
                        let ret = npc.askYesNo("学习#b采矿#k。确定要进行学习吗？需要#b5000金币#k的费用。这些钱你应该有的吧？#b");
                        if (ret == 1) {
                                if (player.hasMesos(5000)) {
                                        ...
                                        npc.say("好了，我已经把采矿的基本知识传授给你了。熟练度积满后提升至下一等级，就可以学习新的内容。等你积满熟练度后，再来找我吧。");
                                } else {
        ...
        case 2:
                if (player.checkMakingSkillExpFull(92010000)) {
                        ...
                        if (ret == 1) {
                                ...
                                player.soundEffect("profession/levelup");
                                player.showAvatarOriented("Effect/OnUserEff.img/professions/equip_product_levelup");
                                npc.sayNext("好了，我已经帮你提升了采矿等级。熟练度积满后可再提升至下一等级，就可以学习新的内容。等你积满熟练度后，再来找我吧。");
                        } else {
        ...
        case 3:
                if (player.getMakingSkillLevel(92020000) > 0) {
                        npc.sayNext("你学习了装备制作，现在无法初始化。真想初始化的话，就得先对装备制作或饰品制作进行初始化。");
                } else {
                        let ret = npc.askYesNo("你想放弃#e采矿#n？是厌倦了吗？之前积累的等级和熟练度……付出的努力和金钱……都将会变成泡影……你真的要初始化吗？");
                        if (ret == 1) {
                                ...
                                npc.sayNext("采矿技术已经初始化。如果想重新学习，请再来找我。");
                        } else {
                                npc.sayNext("看来你很慎重。好的，你先仔细考虑一下，然后再来找我。");
                        }
                }
        ...
        case 4:
                if (!player.hasItem(4011010, 100)) {
                        npc.say("#b#t4011010#100个#k可以交换#i2028067:##b#t2028067#1个#k。请再去搜集一些#t4011010#。");
