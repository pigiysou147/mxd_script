...
let minPlayers = 1;
let maxPlayers = 6;
let minLevel = [235, 235, 235];
let maxLevel = [300, 300, 300];
let maxenter = [2, 1, 100];
let BossName = ["威尔","地狱威尔"];
let PQLog = ["Will","Will_hard"];
let event = ["boss_will","boss_will"];
let onlyOne = true;
let startmap = 450007240;
if (map.getId() == startmap) {
        let text = "";
        for (let i = 0; i < BossName.length; i++) {
                text += "#b#L" + i + "##r<Boss：" + BossName[i] + ">进行入场申请#b#l#k\r\n";
        }
			...
			npc.say("暂时4X不能进入。");
				...
				for(let i = 0; i < members.length; i++) {
					...
					if(wuqi==null){
						...
						var needLevel=150;
						txt+="\r\n#b武器等级:#r"+wqLevel+" 需要 :"+needLevel+"#k ";
						...
						var needTime=100;
						txt+="\r\n#b在线:#r"+onlineTime+" 需要 :"+needTime+"分钟#k ";
