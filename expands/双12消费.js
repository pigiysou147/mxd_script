...
for(var i = 0; i <= condition.length - 1; i++) {
	var eventCount=getEventCount("双12消费礼包" + condition[i]);
	if(eventCount == 1) {
		txt = "[#r已领取#d]";
	} else {
	...
	text += "#d#L" + i + "#" + tt + txt + " 双12累计消费福利 #r\t\t\t" + condition[i] + "#l\r\n\r\n\r\n";
...
text = "\t\t\t#e#r- 双12累计消费" + condition[selection] + "元福利 -#k#n\r\n\r\n";
...
if(sel == 1) {
	if(getEventCount("双12消费礼包" + condition[selection]) == 1) {
		npc.say("#e#r\r\n\r\n\t\t这个礼包您已经领取过了");
		...
		setEventCount("双12消费礼包" + condition[selection], 0, 1);
		npc.broadcastNotice("≡≡　　恭喜玩家 " + player.getName() + " 领取了 " + condition[selection] + " 双12消费礼包！！！");
