...
let selection = npc.askMenu( "我是美发店店长#b#p" + npc.getNpcId() + "##k,只需要1500点券级可以随机换一个脸型。\r\n#b#L0#开始随机选脸型#l\r\n",1012117);
switch (selection) {
        case 0:
                hairs = getStyleChoices(isBeta ? 1 : player.getGender(), player.getHair());
                //res = npc.askAvatar("我能把你现在的脸型变成全新的脸型。你对现在的脸型不厌倦吗?只要你有#b#t5150052##k,我就帮你随机换脸型。慢慢挑选你想要的脸型吧~", hairs, 5150052, isAngel, isBeta);
                item = 5150052;
                take = true;
                styleChange = true;
                break;
...
function modify(selection) {
	if(selection==45240 || selection==32560 ){
		npc.say("请重新随机脸型");
		 ...
		 if (isAngel) {
                player.setSubFace(selection);
                player.modifiedDressUp();
        } else if (isBeta) {
                player.setSubFace(selection);
        } else {
                player.setFace(selection);
        }
