...
txt += "　　#b尊敬的 [ #r#h ##b ] 玩家,这里是降低蓝量系统\r\n";
txt += "　  #d洗蓝将扣除您 [ #r蓝量X20点券的费用白毛是30#d ] #k\r\n";
txt += "您好,欢迎使用点卷购买蓝量上限\r\n" + tz;
txt += "普通职业点卷蓝量比率 #e#r20：1#n#k 蓝量\r\n";
txt += tz + "恶魔复仇者职业点卷蓝量比率\r\n ";
txt += "#e#r30：1#n#k 蓝量#v2000013##v2000013##v2000013#\r\n" + tz;
...
if(job == 3100 || job == 3101 || job == 3120 || job == 3121 || job == 3122 || job == 3101) {
	...
	need = selection * 12;
	let sel = npc.askYesNo("您好,欢迎使用点卷购买降低蓝量.\r\n增加 #r" + selection + "#k 蓝量上限将会使用掉您 #r" + need + " #k点卷\r\n请确认后使用。");
	if(sel == 1) {

		if(player.getCashShopCurrency(1) >= need) {
			player.increaseMaxMp(-hp);
			player.modifyCashShopCurrency(1, -need);
			npc.broadcastNotice("『蓝量上限』 ：恭喜玩家 " + player.getName() + " 使用 " + need + " 点卷降低了 " + hp + " 蓝量上限");
			npc.say("恭喜降低了:" + hp + "蓝量。");
		} else {
