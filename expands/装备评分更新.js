...
function start() {
	...
	for(var i = 0; i < potList.length; i++) {
		...
		if(item != null) {
			...
			//处理神秘徽章
			if(item.getDataId()>=1712001 && item.getDataId()<=1712006){
				item.setStr(item.getArcLevel()*100);
				if(player.getJob()==512){
					//处理队长双倍属性问题
					item.setDex(item.getArcLevel()*0);
					item.setInt(item.getArcLevel()*0);
					item.setLuk(item.getArcLevel()*0);
				}else{
					item.setDex(item.getArcLevel()*100);
					item.setInt(item.getArcLevel()*100);
					item.setLuk(item.getArcLevel()*100);
				}
				
				item.setMaxHp(item.getArcLevel()*50);
				item.setArc(item.getArcLevel()*100);
				player.updateItem(potList[i][0],item);
			}
		} else {
			sql += "," + Single + "," + 0;
		}
	...
	text += "\r\n#L998##fUI/UIAchievement/achievement/pages/main/achievementForm/basic/difficultyIcon/unique#装备评分榜#fUI/UIAchievement/achievement/pages/main/achievementForm/basic/difficultyIcon/unique##l";
	//text += "#L1000##fUI/UIAchievement/achievement/pages/main/achievementForm/basic/difficultyIcon/unique#萌新榜#fUI/UIAchievement/achievement/pages/main/achievementForm/basic/difficultyIcon/unique##l";
	//删榜 再  写榜 
	totalLog(sql);
	player.dropMessage(1,"装备评分更新完毕，当前评分:"+totalScore);
}
...
function equipmentRanking() {
		...
		npc.say(equipmentPosition(selected, characterid)); //装备部位评分	
	
	
}

//装备评分排行榜萌新榜                                                                                                                        
function equipmentRankingMX() {
		var characterid = npc.askMenu(equipmentRankingTextMX());
		//查找出对应的人物                                                                                                                       
		var selected = npc.askMenu(characterEquipInfo(characterid));
		npc.say(equipmentPosition(selected, characterid)); //装备部位评分	
...
function characterEquipInfo(characterid) {
	var sql = "select *,sum(zz_totalscore.`总评分`+challengerstats.str+challengerstats.dex+challengerstats.`int`+challengerstats.luk+challengerstats.pad) total from zz_totalscore,challengerstats where zz_totalscore.characterid = "+characterid+" and zz_totalscore.characterid=challengerstats.characterid";
	var resultList = player.customSqlResult(sql);
	var text = "";
	for(var i = 0; i < resultList.size(); i++) {
		var result = resultList.get(i);
		...
		text = "#e[#r" + result.get("name") + "#k]：装备评分为：" + (result.get("总评分")) + "#k\r\n";
		text+="#r#e力量: "+result.get("str")+"敏捷："+result.get("dex")+" 运气："+result.get("luk")+" 智力 ："+result.get("int")+" 攻击："+result.get("pad")+"#k\r\n";
		text += writeText("帽子", result) + writeText("前额", result) + writeText("脸饰", result) + writeText("耳饰", result) + "\r\n";
		text += writeText("衣服", result) + writeText("短裤", result) + writeText("鞋子", result) + writeText("手套", result) + "\r\n";
		text += writeText("披风", result) + writeText("腰带", result) + writeText("肩部", result) + writeText("符号", result) + "\r\n";
		text += writeText("徽章", result) + writeText("奖牌", result) + writeText("吊坠1", result) + writeText("吊坠2", result) + "\r\n";
		...
		text += writeText("口袋", result) + writeText("机器", result) + writeText("心脏", result) + writeText("图腾1", result) + "\r\n";
		text += writeText("图腾2", result) + writeText("图腾3", result) + writeText("神秘1", result) + writeText("神秘2", result) + "\r\n";
...
function equipmentRankingText() {
	//查出评分表
	//var sql = "select `name`,characterid,jobname,worldid,`总评分` from zz_totalscore where characterid<>1950 and characterid<>1990 and characterid<>31765 and characterid<>4252 and characterid<>22291 ORDER BY 总评分 desc limit 1000";
	var sql ="select zz_totalscore.`name`,zz_totalscore.characterid,zz_totalscore.jobname,zz_totalscore.worldid,sum(zz_totalscore.`总评分`+challengerstats.str+challengerstats.dex+challengerstats.`int`+challengerstats.luk+challengerstats.pad) `总评分` from zz_totalscore,challengerstats where    zz_totalscore.characterid<>31765 and zz_totalscore.characterid<>4252 and zz_totalscore.characterid<>22291 and challengerstats.characterid=zz_totalscore.characterid GROUP BY zz_totalscore.characterid ORDER BY 总评分 desc limit 1000";
	var resultList = player.customSqlResult(sql);
	
	var text = "    #fs18##e" + 勋章 + " #r回忆岛 / #b战力排行#k " + 勋章 + "#fs12#\r\n\r\n";
	text+="   排名    职业       玩家名字       总评分\r\n";
	for(var i = 0; i < resultList.size(); i++) {
		var result = resultList.get(i);
		if(result == null) {
		...
		text += "#L" + result.get("characterid") + "#" + numberString(i + 1) + "" + nbsp10(result.get("jobname")) + "  " + nbsp10(result.get("name")) + "     #r" + result.get("总评分") + "#k#l\r\n";
	
	}
	return text;
}

function equipmentRankingTextMX() {
	//查出评分表
	var sql = "SELECT zz_totalscore.characterid,zz_totalscore.jobname,zz_totalscore.name,zz_totalscore.`总评分` from zz_totalscore,characters,accounts where accounts.id=characters.accounts_id and zz_totalscore.characterid=characters.id and accounts.createdat>'2020-10' order by zz_totalscore.`总评分` desc limit 1000";
	var resultList = player.customSqlResult(sql);
	
	var text = "    #fs18##e" + 勋章 + " #r10月萌新 / #b战力排行#k " + 勋章 + "#fs12#\r\n\r\n";
	text+="   排名    职业       玩家名字       评分\r\n";
	for(var i = 0; i < resultList.size(); i++) {
		var result = resultList.get(i);
		if(result == null) {
