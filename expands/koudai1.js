...
selStr += "#e#r,我可以帮您升级到#v1162032##z1162032#\r\n\r\n";
selStr += "请把#v1162025#放在装备栏的第一格。";
let selection = npc.askYesNo(selStr);
if(selection == 1) {
	var item = player.getInventorySlot(1, 1);
	if(item == null) {
		npc.say("请把#v1162025#放在装备栏的第一格。");
	} else if(item.getDataId() != 1162025) {
		npc.say("请把#v1162025#放在装备栏的第一格。");
	} else {
		var flag = 1;
		for(var i = 0; i < needList.length; i++) {
		...
		if(flag== 1) {
			...
			var toDrop = player.makeItemWithId(1162032);
