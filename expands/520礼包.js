...
for(var i = 0; i < condition.length ; i++) {
    if(getEventCount("520libao" + (i+1)) >= xiangou[i]) {
        txt = "[#r已购买最大值#d]";
    } else {
    ...
    text += "#d#L" + i + "#" + tt + txt + " 520礼包"+(i+1)+" #r\t\t\t" + parseInt(condition[i]*0.35+1) + "#l\r\n\r\n\r\n";
...
text = "\t\t\t#e#r- 520礼包" + parseInt(condition[selection]*0.35+1) + "元福利 -#k#n\r\n\r\n";
...
if(1==1 ){
    if(sel == 1) {
        if(getEventCount("520libao" + (selection+1)) >=xiangou[selection]) {
            npc.say("#e#r\r\n\r\n\t\t这个礼包您已经购买到最大值了");
            ...
            setEventCount("520libao" + (selection+1), 0, 1);
            addHyPay(condition[selection]);
            //npc.broadcastWeatherEffectNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 520大礼包，掌声送给土豪！！！",10000);
            //npc.broadcastPlayerNotice(1,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 520大礼包，掌声送给土豪！！！");
            //npc.broadcastPlayerNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 520大礼包，掌声送给土豪！！！");
            //npc.broadcastPlayerNotice(3,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 520大礼包，掌声送给土豪！！！");
...
function getcountList(){	//														570, 1428, 2857, 5394, 8251,11108
    var sql="select event,sum(value) total from accounts_event where `event` in('520libao1','520libao2','520libao3','520libao4','520libao5','520libao6','520libao7') GROUP BY event";
