...
var avail = "";
avail += "#e#b\t\t\t\t#v1672069#女武神归一#v1672069#\r\n#k";
avail += "#r1、女武神作为本服毕业心脏，能够在基础属性再加上爆炸的属性\r\n";
avail += "2、提升属性需要2个相同的强化到顶级的女武神进行合成\r\n";
...
avail+="\r\n以及金币150亿，1000W点券\r\n";
avail += "#b注意：前两个格子必须放上是强化到达+50的女武神之心   \r\n#k";
avail += "#b当前前3格的装备为:\r\n#r";
if(equip1 != null) {
    ...
    if(equip1 == null) {
        ...
        if(itemid1!=itemid2 ){
            ...
            if(flag == 2) {
				...
				tt+=" 以及 金币150E.点券1000W \r\n\r\n如果需要请点是";
                let sel = npc.askYesNo(tt);

                if(sel == 1) {
                    if(!equip1.getTitle().equals("心脏强化+50") || !equip2.getTitle().equals("心脏强化+50")) {
						npc.say("第一第二个格子的装备必须为强化满了的女武神。");
					}else if(!player.hasMesos(15000000000)){
						npc.say("需要金币150E");
					}else if(player.getCashShopCurrency(1)<10000000) {
						npc.say("你的材料不够 需要 1000W 点券");
						...
						player.modifyCashShopCurrency(1,-10000000);
						player.loseMesos(15000000000);
                        ...
                        //替换掉属性
                        //攻击
                        equip1.setPad(PAD + 200);
                        //魔法攻击
                        equip1.setMad(MAD + 200);
						//伤害
						equip1.setDamR(equip1.getDamR()+50);
						
						equip1.setCraft(1);
                        equip1.setTitle("女武神归一");
						player.loseInvSlot(1,2);
                        player.updateItem(1, equip1);
						
                        npc.broadcastWeatherEffectNotice(0x6, "【女武神归一】  玩家 " + player.getName() + " 对女武神进行了女武神归一  ps:如果归一才是最终的归宿. 那么这把武器我也要归一!!!",15000);
						
						npc.broadcastGachaponMsgEx("【女武神归一】:  玩家 " + player.getName() + " 对女武神进行了女武神归一  ps:如果归一才是最终的归宿. 那么这个顶级心脏我也要归一!!!", equip1);
                ...
                npc.say("请确认你的前两个格子的装备是否为#v1672069#,或者材料是否足够");
