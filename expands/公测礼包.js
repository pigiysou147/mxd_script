...
// 每个阶段礼包所需的充值数
var condition = new Array( 400,1000,2000,4000,6000,12000);
// 礼包内容
var reward = new Array(
	...
	Array(
        ...
        Array(4023025, 35),//金火
        Array(4023026, 80),//紫火
        Array(3992025, 100),//紫火
        Array(5064501, 100),//紫火
        Array(4033298, 60)
    ),
);
...
var xiangou = new Array(3,3,3,2,2,2);

revenue = parseInt(getHyPay(1));
text = "\t\t\t" + tt + " #e公测礼包中心#k#n " + tt + "\r\n\r\n";
...
for(var i = 0; i < condition.length ; i++) {
	var goumai =getEventCount("公测libao" + (i+1));
    if(goumai >= xiangou[i]) {
    ...
    text += "#d#L" + i + "#" + tt + txt + " 公测礼包"+(i+1)+" #r\t\t\t" + parseInt(condition[i]*0.5) + "#l\r\n\r\n\r\n";
...
text = "\t\t\t#e#r- 公测礼包" + parseInt(condition[selection]*0.5) + "元福利 -#k#n\r\n\r\n";
...
if(1==1 ){
    if(sel == 1) {
        if(getEventCount("公测libao" + (selection+1)) >=xiangou[selection]) {
            npc.say("#e#r\r\n\r\n\t\t这个礼包您已经购买到最大值了");
            ...
            setEventCount("公测libao" + (selection+1), 0, 1);
            addHyPay(condition[selection]);
            npc.broadcastWeatherEffectNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.5)+" 公测大礼包，掌声送给土豪！！！",10000);
            npc.broadcastPlayerNotice(1,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.5)+" 公测大礼包，掌声送给土豪！！！");
            npc.broadcastPlayerNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.5)+" 公测大礼包，掌声送给土豪！！！");
            npc.broadcastPlayerNotice(3,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.5)+" 公测大礼包，掌声送给土豪！！！");
...
function getcountList(){	//														570, 1428, 2857, 5394, 8251,11108
    var sql="select event,sum(value) total from accounts_event where `event` in('公测libao1','公测libao2','公测libao3','公测libao4','公测libao5','公测libao6','公测libao7') GROUP BY event";
