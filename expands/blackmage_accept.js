...
let minPlayers = 1;
let maxPlayers = 6;
let minLevel = [245, 245, 245, 245];
let maxLevel = [300, 300, 300, 300];
let maxenter = [2, 2,10,10];
let BossName = ["黑魔法师","深渊黑魔法师","黑魔法师(竞速不限次数无奖励)","深渊黑魔法师(竞速不限次数无奖励)"];
let PQLog = ["BlackMage","BlackMage_hard","BlackMage_test","BlackMage_hard_test" ];
let event = ["boss_blackmage","boss_blackmage","boss_blackmage","boss_blackmage"];
let onlyOne = true;
...
if (map.getId() == startmap) {
        ...
        let sel = npc.askMenuE("为了与黑魔法师对峙.要前往#b黑暗神殿#k吗？\r\n" + text, true);
        if (party == null || player.getId() != party.getLeader()) {
				...
				for(let i = 0; i < members.length; i++) {
					...
					if(accountCount>=9 || accountTodayCount>=3){
						flag=false
					}
				...
				if(flag == true) {
					...
					if(choos == 1) {
						if(npc.makeEvent(event[sel], onlyOne, [party,sel]) == null) {
        ...
        let ret = npc.askYesNoE("#e<Boss：黑魔法师>#n\r\n\r\n你想要现在就离开这里吗？", true);
        if (ret == 1) {
                player.changeMap(450012500);
        }
}
