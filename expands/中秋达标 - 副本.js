...
// 每个阶段礼包所需的充值数
var condition = new Array(1000,3000,5000,10000);
// 礼包内容
...
for(var i = 0; i < condition.length ; i++) {
	...
	var goumai =getEventCount("zqgq" + (i+1));
    if(goumai >= xiangou[i]) {
    ...
    text += "#d#L" + i + "#" + tt + txt + " 中秋国庆达标福利"+(i+1)+" #r\t" + price + "#l\r\n\r\n\r\n";
	var cc=4;
...
if(sel==1){
	if(getEventCount("zqgq" + (selection+1)) >=xiangou[selection]) {
		npc.say("#e#r\r\n\r\n\t\t这个礼包您已经购买到最大值了");
		...
		setEventCount("zqgq" + (selection+1), 0, 1);
		
		npc.broadcastWeatherEffectNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection])+" 中秋国庆礼包，掌声送给土豪！！！",10000);
		npc.broadcastPlayerNotice(1,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection])+"  中秋国庆礼包，掌声送给土豪！！！");
		npc.broadcastPlayerNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection])+"  中秋国庆礼包，掌声送给土豪！！！");
		npc.broadcastPlayerNotice(3,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection])+"  中秋国庆礼包，掌声送给土豪！！！");
		npc.say("领取成功");
	}
}
function getcountList(){	
	var sql="select event,sum(value) total from accounts_event where `event` in('zqgq1','zqgq2','zqgq3','zqgq4') GROUP BY event";
...
function getHyPay() {
    var sql = "SELECT sum(rmb) total from paylog where account=? and paytime>'2021-09-19' and paytime<'2021-10-08'";
