...
let selects = npc.askNumber("\r\n#d想当门派老大么？ 那么如果您以每 #r" + cost + " #d点卷增加1点门派贡献度。贡献度达到第一的时候那么您就可以出任门派老大了。\r\n\r\n#b\t\t\t" + zzz + "目前您的点卷：#r" + player.getCashShopCurrency(1) + " #k#b\r\n\t\t\t" + zzz + "门派贡献度为：#r" + getMenpaiMemberPoints() + "\r\n\r\n 请问你想购买多少贡献度?　", 1, 1, player.getCashShopCurrency(1));

let sel = npc.askYesNo("您确定购买贡献度吗？\r\n增加 #r" + selects + "#k 点贡献度将会使用掉您 #r" + selects * cost + "#k 点卷\r\n");
if(sel == 1) {
	if(player.getCashShopCurrency(1) >= selects * cost) {
		player.modifyCashShopCurrency(1, -selects * cost);
		AddMenpaiMemberPoints(selects)
		npc.broadcastPlayerNotice(0x05, "[门派贡献度] ：恭喜玩家 " + player.getName() + " 使用用 " + selects * cost + " 点卷增加了 " + selects + " 门派个人贡献度。");
