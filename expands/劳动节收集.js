...
var totalCount=getEventCount("51捐赠");
var sql="select sum(value) total from accounts_event where `event`='51捐赠'";
var worldCount=player.customSqlResult(sql).get(0).get("total");
var txt="\t\t\t#r#e#v"+need+"#五一劳模大评比#v"+need+"#\r\n\r\n";
txt+=" 你好，我是一个勤劳的打工人。。。\r\n\r\n";
txt+="你可以收集#v"+need+"#来捐赠给我，我可以给你打工人应有的报酬哦~\r\n\r\n";
		...
		if(item==null){
			...
			if(ch==0){
				...
				if(worldCount>500000){
					if(getEventCount("五一收集50万奖励")==0){
						//5W的奖励
						
						addHyPay(100);
						setEventCount("五一收集50万奖励",0,1)
						npc.say("领取成功");
					}else{
						npc.say("请不要重复领取");
					}
