...
var menuList = Array(
	...
	Array("#r使用#v2430078#(花費1張報告書)強化符文" , 3)
        );
var 紙條 = 2430078; 
var 數量 = 1; 
...
var chibangList = Array(
	...
	Array(3 ,1162041,40	,25	,142),
	Array(4 ,1162042,80 	,20	,166),
...
let selStr = "";
selStr +="#b#r"+eff+"餘額商城購買#v2430078#*1來強化你的口袋道具吧\r\n";
			...
			if(nowLevel >= chibangList.length ){
				...
				if(YN == 1){
					if(player.hasItem(紙條, 數量)){
						...
						npc.broadcastGachaponMsgEx("【符文系統】:恭喜 玩家「"+player.getName()+"」 使用防爆升級成功將符文進化到 階段" + (nowLevel+1)+"",toDrop);
						npc.say("升級成功");
						}else{
							player.loseItem(紙條, 數量);
							npc.broadcastPlayerNotice(28, "[好險沒爆] :  「" + player.getName() + "」嘗試將口袋道具 提升至" +(nowLevel+1) + "階 失敗，但是道具保留。");
							npc.say("強化失敗 已保留符文");
						}
					}else{
						npc.say("您的材料不足，無法升級");
