...
let maxenter = [1, 1];
let BossName = ["普通模式<皮埃尔>", "进阶模式<皮埃尔>"];
let PQLog = ["Pierre", "Pierre_CHAOS"];
...
if (map.getId() == startmap) {
        ...
        let sel = npc.askMenu("#e<鲁塔比斯西侧庭院入口>#n\r\n\r\n#b#h0# \n\#k通往鲁塔比斯西侧封印的守护者 #r皮埃尔#k守卫的庭院的门。 #r完成记录将在晚12点初始化。#b\r\n" + text, 1064012);
        if (party == null || player.getId() != party.getLeader()) {
                npc.sayNext("你需要有一个#r" + minPlayers + "~" + maxPlayers + "#k人的队伍.并且等级在" + minLevel[sel] + "~" + maxLevel[sel] + "范围,\r\n那么请让你的组队长和我对话吧!", 1064012);
        } else if (party.numberOfMembersInChannel() < minPlayers || party.getMembersCount(map.getId(), 1, 300) > maxPlayers) {
                npc.sayNext("你需要有一个#r" + minPlayers + "~" + maxPlayers + "#k人的队伍.!", 1064012);
        } else if (party.getMembersCount(map.getId(), minLevel[sel], maxLevel[sel]) < minPlayers) {
                npc.sayNext("你队员的等级要在" + minLevel[sel] + "~" + maxLevel[sel] + "范围!请确认!", 1064012);
        } else if (party.getMembersCount(map.getId(), 1, 300) < party.getMembersCount()) {
                npc.sayNext("好像有队员在其他地方，请把队员都召集到这里！", 1064012);
        } else if (!party.isAllMembersAllowedPQ(PQLog[sel], maxenter[sel])) {
                npc.sayNext("你队员#r#e" + party.getNotAllowedMember(PQLog[sel], maxenter[sel]).getName() + "#n#k次数已经达到上限了。", 1064012);
        } else if (npc.makeEvent(event[sel], onlyOne, party) == null) {
                npc.sayNext("已经有队伍在进行了,请更换其他频道尝试。", 1064012);
        }
} else {
        let ret = npc.askYesNoS("#e<Boss：皮埃尔>#n\r\n\r\n你想要现在就离开这里吗？");
        if (ret == 1) {
