...
function getDays(){
	var sql = "select TO_DAYS(now())-TO_DAYS(createdat) day from accounts where id=?";

	var result = player.customSqlResult(sql, player.getAccountId());
	
	var days =result.get(0).get("day");
	if(days<1){
		return 1;
	}else{
		return days ;
...
function getMylower() {
	...
	var sql = "select * from qqmxd_register where promote_id=?  ";
	var result = player.customSqlResult(sql,player.getAccountId());
	return result;
