...
function start() {
	...
	sql += " a.itemid,a.meso,a.iid,a.number FROM zz_js_cailiao_history a where characters_id = ? order by iid desc "
	...
	for(var i = 0; i < resultList.size(); i++) {
		...
		if(result.get("status") == "已售出") {
			text += result.get("iid") + "#r#L" + result.get("iid") + "##v" + result.get("itemid") + "#\t\t￥" + result.get("meso") + "\t\t" + result.get("sale") + "\t   " + (result.get("buy") == null ? "-" : result.get("buy")) + "\t#b领取现金#k#l\r\n";
		} else if(result.get("status") == "正在出售") {
			text += result.get("iid") + "#r#L" + result.get("iid") + "##v" + result.get("itemid") + "#\t\t￥" + result.get("meso") + "\t\t" + result.get("sale") + "\t   " + (result.get("buy") == null ? "-" : result.get("buy")) + "\t#r装备下架#k#l\r\n";
		} else if(result.get("status") == "已下架") {
			text += result.get("iid") + "#r#L" + result.get("iid") + "##v" + result.get("itemid") + "#\t\t￥" + result.get("meso") + "\t\t" + result.get("sale") + "\t   " + (result.get("buy") == null ? "-" : result.get("buy")) + "\t#r已下架#k#l\r\n";
		} else if(result.get("status") == "领过现金") {
			text += result.get("iid") + "#r#L" + result.get("iid") + "##v" + result.get("itemid") + "#\t\t￥" + result.get("meso") + "\t\t" + result.get("sale") + "\t   " + (result.get("buy") == null ? "-" : result.get("buy")) + "\t#r领过现金#k#l\r\n";
...
function findEquipAttr(iid) {
	var text = "";
	var sql = " SELECT * from zz_js_cailiao_history where iid = ? limit 1 ";
		...
		if(result.get("buy_id") > 0 && 　result.get("status") == "已售出") {
			...
			if(sel == 1) {
				if(player.canGainItem(result.get("itemid"), result.get("number"))) {
					...
					var sql = " SELECT * from zz_js_cailiao where iid = ? limit 1 ";
					var resultList = player.customSqlResult(sql, iid);
...
function removeSale(iid) {
	var sql = " delete from zz_js_cailiao where iid = ? ";
	player.customSqlUpdate(sql, iid);
}

function modifyBuyIdRMB(iid) {
	var sql = " update zz_js_cailiao_history set status = '领过现金' where iid = ? ";
	player.customSqlUpdate(sql, iid);
}

function modifyBuyIdStatus(iid) {
	var sql = " update zz_js_cailiao_history set status = '已下架' where iid = ? ";
	player.customSqlUpdate(sql, iid);
}
