			...
			if(YN == 1){
				if(PD){
					...
					if(ran <= nowArray[2]){
						...
						wk = nowArray[1];
						makeEquip(true);
						//npc.broadcastGachaponMsgEx("【GM上星】 : 恭喜管理 " + player.getName() + " 上星 "+toDrop.getItemName()+" 成功！",toDrop);
						npc.say("恭喜你升星成功");
					}else if(ran <= nowArray[2]+nowArray[3]){
						//失敗
						//npc.broadcastGachaponMsgEx("【GM上星】 : 管理 " + player.getName() + " 上星 "+toDrop.getItemName()+" 失敗！",toDrop);
						npc.say("很遺憾，升星失敗");
					}else{
						//下降
						ap = lastArray[0];
						wk = lastArray[1];
						makeEquip(false);
						npc.broadcastGachaponMsgEx("【GM上星】 : 真糟糕,管理 " + player.getName() + " 上星 "+toDrop.getItemName()+" 失敗！並且星級下降！",toDrop);
