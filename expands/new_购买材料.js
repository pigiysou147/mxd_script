...
function start(){
	...
	var sql = " SELECT iid, a.itemid, sn, a.meso,number FROM zz_js_cailiao a WHERE characters_id<>? and type = 2 AND buy_id IS NULL ORDER BY paytime DESC ";
	...
	if(yes == 1){
		if (!player.canGainItem(itemid, number)) {
			...
			var sql = " SELECT * FROM zz_js_cailiao WHERE iid=? ";
...
function modifyBuyId(iid){
	var sql = " update zz_js_cailiao_history set buy_id = ?,status='已售出' where iid = ? ";
