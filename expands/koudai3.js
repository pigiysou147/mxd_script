...
selStr += "#e#r,我可以帮您升级到#v1162037##z1162037#\r\n\r\n";
selStr += "请把#v1162035#放在装备栏的第一格。";
let selection = npc.askYesNo(selStr);
if(selection == 1) {
	var item = player.getInventorySlot(1, 1);
	if(item == null) {
		npc.say("请把#v1162035#放在装备栏的第一格。");
	} else if(item.getDataId() != 1162035) {
		npc.say("请把#v1162035#放在装备栏的第一格。");
	} else {
		var flag = 1;
		for(var i = 0; i < needList.length; i++) {
		...
		if(flag == 1) {
			...
			var toDrop = player.makeItemWithId(1162037);
