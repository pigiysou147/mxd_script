...
var menuList = Array(
	...
	Array("#r使用#v2430078#(花費1張報告書)強化徽章" , 3)
        );
var 紙條 = 2430078; 
var 數量 = 1; 
...
var chibangList = Array(
	...
	Array(2 ,1182002,20	,10,125,10),
	Array(3 ,1182003,40	,20,142,20),
	Array(4 ,1182004,80,25,  166,25),
...
switch (selected){
	case 1:
		...
		ddd += "#r\t1. #v"+chibangList[0][1]+"# "+chibangList[0][0]+"  階符文 "+chibangList[0][2]+" 屬性 ,免費領取\r\n";
		for(let i = 1; i<chibangList.length ;i++){
			ddd += "#r\t"+(i+1)+". #v"+chibangList[i][1]+"# "+chibangList[i][0]+"  階符文 "+chibangList[i][2]+" 屬性 ,所需#v"+紙條+"# x"+數量+"\r\n";
		}
		npc.say(ddd);
		break;
	case 2:
		...
		if(player.hasItem(1182001, 1)){
			...
			npc.say("領取成功，如果不慎失去了徽章，請聯系管理員獲取");
		}
		break;
	case 3:
		//獲取第一格裝備
			...
			if(nowLevel >= chibangList.length ){
				...
				if(YN == 1){
					if(player.hasItem(紙條, 數量)){
						...
						npc.broadcastGachaponMsgEx("【符文系統】:恭喜 玩家「"+player.getName()+"」 使用防爆升級成功將徽章進化到 階段" + (nowLevel+1)+"",toDrop);
						npc.say("升級成功");
						}else{
							player.loseItem(紙條, 數量);
							npc.broadcastPlayerNotice(28, "[好險沒爆] :  「" + player.getName() + "」嘗試將徽章 提升至" +(nowLevel+1) + "階 失敗，但是道具保留。");
							npc.say("強化失敗 已保留徽章");
						}
					}else{
						npc.say("您的材料不足，無法升級");
