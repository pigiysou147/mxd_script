...
var xiangou = new Array(1,3,5,6,6,6);

revenue = parseInt(getHyPay(1));
text = "\t\t\t" + tt + " #e双12欢乐购礼包中心#k#n " + tt + "\r\n\r\n";
text += "#d#e当前可用于购买礼包的金额为： #r" + revenue + " #d元\r\n";
var zhekou=0;
if(player.getPQLog("年会员")>=1){
	zhekou=1;
	text += "#e#r尊贵的永久会员，本次双12礼包你可以享受9折购买权限#k\r\n";
}
...
for(var i = 0; i < condition.length ; i++) {
	...
	var goumai =getEventCount("shuang12" + (i+1));
    if(goumai >= xiangou[i]) {
    ...
    text += "#d#L" + i + "#" + tt + txt + " 双12欢乐购礼包"+(i+1)+" #r\t" + price + "#l\r\n\r\n\r\n";
	var cc=4;
...
if(sel == 1) {
	if(getEventCount("shuang12" + (selection+1)) >=xiangou[selection]) {
		npc.say("#e#r\r\n\r\n\t\t这个礼包您已经购买到最大值了");
		...
		1for(var i = 0; i < reward[selection].length; i++) {
			player.gainItem(reward[selection][i][0], reward[selection][i][1]);
		}
		setEventCount("shuang12" + (selection+1), 0, 1);
		addHyPay((need/0.35)-1);
		npc.broadcastWeatherEffectNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 双12欢乐购礼包，掌声送给土豪！！！",10000);
		npc.broadcastPlayerNotice(1,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 双12欢乐购礼包，掌声送给土豪！！！");
		npc.broadcastPlayerNotice(2,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 双12欢乐购礼包，掌声送给土豪！！！");
		npc.broadcastPlayerNotice(3,"恭喜玩家"+player.getName()+"购买了价值"+parseInt(condition[selection]*0.35+1)+" 双12欢乐购礼包，掌声送给土豪！！！");
...
function getcountList(){	
	var sql="select event,sum(value) total from accounts_event where `event` in('shuang121','shuang122','shuang123','shuang124','shuang125','shuang126') GROUP BY event";
