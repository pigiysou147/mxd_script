...
for(var i = 0; i <= condition.length - 1; i++) {
	if(getEventCount("520消费礼包" + condition[i]) == 1) {
		txt = "[#r已领取#d]";
	} else {
		if(getEventCount("520消费礼包" + condition[i]) == 0 && revenue >= condition[i]) {
			txt = "[#b待领取#d]";
		} else if(getEventCount("520消费礼包" + condition[i]) == 0 && revenue < condition[i]) {
			txt = "[#g未达标#d]";
		}

	}
	text += "#d#L" + i + "#" + tt + txt + " 520累计消费福利 #r\t\t\t" + condition[i] + "#l\r\n\r\n\r\n";
...
text = "\t\t\t#e#r- 520累计消费" + condition[selection] + "元福利 -#k#n\r\n\r\n";
...
if(sel == 1) {
	if(getEventCount("520消费礼包" + condition[selection]) == 1) {
		npc.say("#e#r\r\n\r\n\t\t这个礼包您已经领取过了");
		...
		setEventCount("520消费礼包" + condition[selection], 0, 1);
