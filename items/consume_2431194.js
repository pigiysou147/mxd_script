...
let newerEquips = Array(
	//裝備ID 裝備名 四圍 攻擊力 BOSS 所屬 無視 總傷害
	Array(1012268,"首儲裝備",500,500,50,0,50,50,"全能力+500")
);
...
if (newerEquips.length > 0){
	...
	for(let i = 0 ; i < newerEquips.length;i++){
		ss += "#v"+newerEquips[i][0]+"##r#e"+newerEquips[i][8]+""
	}
}
...
let YN = npc.askYesNo("#b是否領取#r全屬+500#v1012268#的7天冰棒？#b"+ss);
if(YN == 1){
	...
	if (player.getFreeSlots(1) < needSlot || player.getFreeSlots(2) < needSlot || player.getFreeSlots(3) < needSlot || player.getFreeSlots(4) < needSlot || player.getFreeSlots(5) < needSlot) {
		...
		for(let i = 0 ; i < newerEquips.length;i++){
			...
			newItem.setDamR(newerEquips[i][7]);// 總傷害
			newItem.setAttribute(0);
			newItem.setExpiration(new Date().getTime() + (1000*60*60*24*7));
			player.gainItem(newItem);
			npc.broadcastGachaponMsgEx("【首儲禮包】:恭喜 ［"+player.getName()+"］ 領取了首次儲值的限時裝備!",newItem);
		}
