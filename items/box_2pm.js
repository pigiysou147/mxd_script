...
var xiangou = new Array(2,2,2,2,2,2,2);
...
for(var i = 0; i < condition.length ; i++) {
	if(getEventCount("个人礼包" + (i+1)) >= xiangou[i]) {
		txt = "[#r已购买最大值#d]";
	} else {
...
var week =cal.DAY_OF_WEEK;
if(1==1 ){
	if(sel == 1) {
		if(getEventCount("个人礼包" + (selection+1)) >=xiangou[selection]) {
			npc.say("#e#r\r\n\r\n\t\t这个礼包您已经购买到最大值了");
			...
			setEventCount("个人礼包" + (selection+1), 0, 1);
			addHyPay(condition[selection]);
			npc.say("领取成功");
		}
	}
}else{
	npc.say("只有周一到周五才开放购买哦");
}
