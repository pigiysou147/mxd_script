...
let newerEquips = Array(
	...
	Array(1112918,"大斗乾爹專屬",1688,1688,0,0,0,"全能力+1688")
);
...
let YN = npc.askYesNo("#b是否領取#r全屬+1688#v1112918#戒指？#b"+ss);
if(YN == 1){
	...
	if (player.getFreeSlots(1) < needSlot || player.getFreeSlots(2) < needSlot || player.getFreeSlots(3) < needSlot || player.getFreeSlots(4) < needSlot || player.getFreeSlots(5) < needSlot) {
		...
		npc.broadcastGachaponMsgEx("【大斗乾爹】:感謝 "+player.getName()+" 乾爹贊助伺服器，並且領取了1688全屬80%戒指",newItem);
		npc.broadcastGachaponMsgEx("【大斗乾爹】:感謝 "+player.getName()+" 乾爹贊助伺服器，並且領取了1688全屬80%戒指",newItem);
		npc.broadcastGachaponMsgEx("【大斗乾爹】:感謝 "+player.getName()+" 乾爹贊助伺服器，並且領取了1688全屬80%戒指",newItem);
		npc.broadcastGachaponMsgEx("【大斗乾爹】:感謝 "+player.getName()+" 乾爹贊助伺服器，並且領取了1688全屬80%戒指",newItem);
		npc.broadcastGachaponMsgEx("【大斗乾爹】:感謝 "+player.getName()+" 乾爹贊助伺服器，並且領取了1688全屬80%戒指",newItem);
