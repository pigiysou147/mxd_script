...
function action(){
	map.blowWeatherEffectNotice("组队副本正式开始!勇敢的男人冲啊！", 102, 5000);
	state=1;
...
function mobDied(mob) {
	times++;
	if(times==2){
		map.blowWeatherEffectNotice("现在进入第二波攻势！！！", 102, 5000);
				
		var mob3 = map.makeMob(9833101);
		mob3.changeBaseHp(80000000000000*2);
		map.spawnMob(mob3,  0, 153);

		var mob4 = map.makeMob(8645039);//吨凯尔
		mob4.changeBaseHp(80000000000000+80000000000000);////
		map.spawnMob(mob4, 0, 153);//
 
	}else if(times==4){
		map.blowWeatherEffectNotice("现在进入最后一波攻势！！！大家加油呀，胜利就在前方 ！", 102, 5000);
				
		var mob5 = map.makeMob(9833101);
		mob5.changeBaseHp(80000000000000*3);
		map.spawnMob(mob5,  0, 153);

		var mob6 = map.makeMob(8645039);//吨凯尔
		mob6.changeBaseHp(80000000000000*3);////
		map.spawnMob(mob6, 0, 153);//
		
		
		var mob7 = map.makeMob(8880342);//品客宾
		mob7.changeBaseHp(80000000000000);////
		map.spawnMob(mob7,  0, 153);
		...
		for(var i = 0; i<damList.length;  i++){ 
			...
			if(i==0){
				event.broadcastPlayerNotice(7, "[远征伤害排名] 排名:"+(i+1)+"  玩家："+player.getName()+"  伤害 ："+damList[i][0]+"  伤害占比 : "+parseFloat(damList[i][0]/datotal)*100+" %"   );
